...
double low;
double all_distance;
extern int contract=316.99; //micro 0.01 THB
int cash = 30000; //THB
extern int zone = 8; // zone big picture
extern int max_leverage=2; 
extern int max_bullet=10;
...
double pipval=0.03; //micro 0.01 THB
double last_price;
...
//---
   ...
   if (new_bar) {
      ...
      //check can open more
      can_open = false;
      if(total_order==0 || (current_zone != 0 && current_zone_mini != 0 && pos[current_zone_mini]==0 && pos[0] < bullet)){
         can_open=true;
      }
   }
      ...
      if(Order!=SIGNAL_NONE)
        {
         ...
         Print("stoploss = " + stoploss_distance);
         //TakeProfit = zone_mini_distance;
         if(Order==SIGNAL_BUY)
           {
            Lots=calLots();
            Print("lots = " + Lots);
            ...
            Ticket=OrderSend(Symbol(),OP_BUY,Lots,NormalizeDouble(Ask, Digits),Slippage,NormalizeDouble(StopLossLevel, Digits),NormalizeDouble(TakeProfitLevel, Digits),"Buy(#"+MagicNumber+")",MagicNumber,0,DodgerBlue);
            if(Ticket>0)
              {
      ...
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
        {
         ...
         //TakeProfitLevel = Ask+newTP*Point*P;
         OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(StopLossLevel, Digits),OrderTakeProfit(),0);
        }
     }
  }
//+------------------------------------------------------------------+
   ...
   for(int i=0;i<total_order;i++)
     {
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) 
        {
         if(OrderSymbol() == Symbol() && current_zone == findCurrentlyZone(OrderClosePrice())){
...
bool checkPosition()
  {
   ...
   for(int i=0;i<total_order;i++){
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
         if(OrderSymbol() == Symbol() && current_zone==findCurrentlyZone(OrderOpenPrice()))
            return true;
   ...
   for(int i=0;i<=bullet;i++)
     {
      if(i==0)
         ...
         zone_mini_price[bullet] = NormalizeDouble(zone_mini_high, Digits);
      else
...
void setMM()
  {
   ...
   double new_sl;
   //double max_sl = (zone_distance*4)/Point; //4 zone
   double max_sl = 3*std;
      ...
      for(int i=max_bullet;i>=1;i--)
        {
         ...
         new_sl = risk_per_trade / (pipval * i);
         leverage = (contract * i) / risk_per_trade;
         if(leverage < max_leverage && new_sl > max_sl){
            find_bullet = True;
            bullet = i;
            break;
         } else if (i == 0) {
            Alert("Cannot find Bullet");
            break;
         }
        }
     }
...
void setGrid()
  {
   ...
   for(int i=0;i<=zone;i++){
      if(i==0){
         ...
         zone_price[zone]=NormalizeDouble(high, Digits);
      } else {
...
void printScreen()
  {
   ...
   ObjectSetText("sd","3 SD: "+std*3,font_size,"Verdana",White);
...
int testSignal()
  {
   ...
   double emaSlow_1 = iMA(NULL, 0, 50, 0, MODE_SMA, PRICE_CLOSE, 1);
   double emaSlow_2 = iMA(NULL, 0, 50, 0, MODE_SMA, PRICE_CLOSE, 2);
   maBias=iMA(NULL,PERIOD_D1,5,0,MODE_EMA,PRICE_CLOSE,1);

   if(last_price > maBias && iLow(NULL,PERIOD_CURRENT,1)<emaFast_1 && iClose(NULL,PERIOD_CURRENT,1)>emaFast_1 && emaFast_1>emaSlow_1) 
     {
      return SIGNAL_BUY;
     }
